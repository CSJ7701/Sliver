...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Load the 'gui' sliver if you are on a machine with hostname 'Jimmy' or 'Johnny' and using the 'x' window system
  ;; Note that, when passing a list to a predicate, it must be quoted.
  (sliver-load "gui"
  	     :hostname '("Jimmy" "Johnny")
  	     :window-system 'x)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Lists must also be quoted.

  (setq sliver-machine-profiles
        '(("Home-PC" . (:hostname "Jimmy" :os gnu/linux))
  	  ("Machines-with-Firefox" . (:hostname '("Billy" "Bobby")))))

  (sliver-load "desktop" :profile "Home-PC")
  (sliver-load "firefox" :profile "Machines-with-Firefox")
