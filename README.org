...
/Note:/ ChatGPT taught me a new word when I asked it to proofread this README. Now you get to learn it too :D

_Idempotent_ (adjective)
- (mathematics) Relating to or being a mathematical quantity which, when applied to itself under a given binary operation, equals itself
- (computing) Describing an action which, when performed multiple times, has no further effect on its subject after the first time


** Metadata
Sliver allows modules to conflict with, or depend on, other modules. This functionality is supported through the use of header metadata in each sliver file (these are just elisp comments). Metadata should look like this:
...
The =sliver-declare-dependency= function is also interactive. It will prompt for each module if not provided.
...
The =sliver-declare-conflict= function is also interactive. It will prompt for each module if not provided.
