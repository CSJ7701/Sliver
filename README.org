...
/Note:/ ChatGPT taught me a new word when I asked it to proofread this README. Now you get to learn it too
...
Dependencies are strict: if a dependency cannot be loaded, the requesting sliver will not load. Dependencies are _unidirectional_; if X is dependent on Y, Y is not necessarily dependent on X.
...
Conflicts are tracked globally and symmetric, meaning that if X declares a conflict with Y, then Y must also conflict with X.
